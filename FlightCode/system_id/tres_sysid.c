...
extern void get_system_id( double time, struct sensordata *sensorData_ptr, struct nav *navData_ptr, struct control *controlData_ptr, struct mission *missionData_ptr){
	...
	// 3-2-1-1 Roll on ganged then surface pairs
		...
		if((time-t0)<6.5){
			...
			controlData_ptr->roll_cmd_excite = doublet3211(0+t0, time, 0.175, 3.5*D2R); 	// total duration is 7x duration input
			controlData_ptr->da = controlData_ptr->da + controlData_ptr->roll_cmd_excite;
		...
		else if(((time-t0)>=6.5)&&((time-t0)<11.5)){
			// 3-2-1-1 antisymmetric excitations at 2 Hz to ailerons
			controlData_ptr->roll_cmd_excite = doublet3211(7+t0, time, 0.175, 7*D2R); 	// total duration is 7x duration input
			controlData_ptr->da = controlData_ptr->da + controlData_ptr->roll_cmd_excite;
		}
