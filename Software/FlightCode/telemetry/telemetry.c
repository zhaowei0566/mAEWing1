...
#define TELE_PACKET_SIZE 18 // 16-bit telemetry packet bytes
...
void send_telemetry(struct sensordata *sensorData_ptr, struct nav *navData_ptr, struct control *controlData_ptr, struct mission *missionData_ptr, uint16_t cpuLoad)
{
	...
	//tele_data[5] = (uint16_t)( sensorData_ptr->imuData_ptr->ax / 100.0 * 0x7FFF );	// IMU accels = 100 m/sec2
	//tele_data[6] = (uint16_t)( sensorData_ptr->imuData_ptr->ay / 100.0 * 0x7FFF );
	tele_data[3] = (uint16_t)( sensorData_ptr->imuData_ptr->az / 100.0 * 0x7FFF );
	
	tele_data[4] = (uint16_t)( (sensorData_ptr->adData_ptr->h) / 10000.0 * 0x7FFF );	// max AGL Alt.(m) = 10000 m
	tele_data[5] = (uint16_t)( (sensorData_ptr->adData_ptr->ias) / 80.0 * 0x7FFF );	// max Indicated Airspeed(IAS) = 80 m/s	

	tele_data[6] = (uint16_t)( navData_ptr->psi*R2D / 180.0 * 0x7FFF );	// Euler angles [psi,theta,phi] (deg)
	tele_data[7]= (uint16_t)( navData_ptr->the*R2D / 90.0 * 0x7FFF );
	tele_data[8]= (uint16_t)( navData_ptr->phi*R2D / 180.0 * 0x7FFF );

	//tele_data[13]= (uint16_t)( controlData_ptr->l1 / L1_MAX * 0x7FFF );	// control surface commands (normalized 0-1)
	tele_data[9]= (uint16_t)( controlData_ptr->l2 / L2_MAX * 0x7FFF );
	tele_data[10]= (uint16_t)( controlData_ptr->l3 / L3_MAX * 0x7FFF );
	tele_data[11]= (uint16_t)( controlData_ptr->l4 / L4_MAX * 0x7FFF );
	//[17]= (uint16_t)( controlData_ptr->r1 / R1_MAX * 0x7FFF );
	tele_data[12]= (uint16_t)( controlData_ptr->r2 / R2_MAX * 0x7FFF );
	tele_data[13]= (uint16_t)( controlData_ptr->r3 / R3_MAX * 0x7FFF );
	tele_data[14]= (uint16_t)( controlData_ptr->r4 / R4_MAX * 0x7FFF );
	tele_data[15]= (uint16_t)( controlData_ptr->dthr * 0x7FFF );
	
	//tele_data[22]= (uint16_t)( sensorData_ptr->accelData_ptr->cf / 8.0 * 0x7FFF );	// accelerometers = 8 g
	//tele_data[23]= (uint16_t)( sensorData_ptr->accelData_ptr->cr / 8.0 * 0x7FFF );
	//tele_data[24]= (uint16_t)( sensorData_ptr->accelData_ptr->lf / 8.0 * 0x7FFF );
	//tele_data[25]= (uint16_t)( sensorData_ptr->accelData_ptr->lr / 8.0 * 0x7FFF );
	//tele_data[26]= (uint16_t)( sensorData_ptr->accelData_ptr->rf / 8.0 * 0x7FFF );
	//tele_data[27]= (uint16_t)( sensorData_ptr->accelData_ptr->rr / 8.0 * 0x7FFF );
	
	//tele_data[28] = cpuLoad;		/* cpuload */

	//tmp = (unsigned long)( sensorData_ptr->gpsData_ptr->lon *1e07 );
	//memcpy(&tele_data[29],&tmp,4);
	//tmp = (unsigned long)( sensorData_ptr->gpsData_ptr->lat *1e07 );
	//memcpy(&tele_data[31],&tmp,4);
	...
	tele_data[17] = (uint16_t)sensorData_ptr->gpsData_ptr->satVisible;
	
	
	memcpy(&sendpacket[3], &tele_data, SEND_PACKET_SIZE-5);
	// compute 16-bit checksum of output data (excluding the header)
	output_CKSUM = do_chksum (sendpacket, 2, SEND_PACKET_SIZE-2);
	*(uint16_t *)&sendpacket[SEND_PACKET_SIZE-2] = output_CKSUM;

	// send send_telemetry data packet to serial port
	while(bytes != SEND_PACKET_SIZE) bytes += write(port, &sendpacket[bytes], TELE_PACKET_SIZE-bytes); bytes=0;
