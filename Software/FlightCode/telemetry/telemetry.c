...
#define TELE_PACKET_SIZE 35 // 16-bit telemetry packet bytes
#define SEND_PACKET_SIZE 75 // 8-bit telemetry packet = [<UUT>, tele_data, <16bit_CKSUM>]
...
void send_telemetry(struct sensordata *sensorData_ptr, struct nav *navData_ptr, struct control *controlData_ptr, struct mission *missionData_ptr, uint16_t cpuLoad)
{
	...
	tele_data[7] = (uint16_t)( sensorData_ptr->imuData_ptr->az / 100.0 * 0x7FFF );
	
	tele_data[8] = (uint16_t)( (sensorData_ptr->adData_ptr->h) / 10000.0 * 0x7FFF );	// max AGL Alt.(m) = 10000 m
	tele_data[9] = (uint16_t)( (sensorData_ptr->adData_ptr->ias) / 80.0 * 0x7FFF );	// max Indicated Airspeed(IAS) = 80 m/s	

	tele_data[10] = (uint16_t)( navData_ptr->psi*R2D / 180.0 * 0x7FFF );	// Euler angles [psi,theta,phi] (deg)
	tele_data[11]= (uint16_t)( navData_ptr->the*R2D / 90.0 * 0x7FFF );
	tele_data[12]= (uint16_t)( navData_ptr->phi*R2D / 180.0 * 0x7FFF );

	tele_data[13]= (uint16_t)( controlData_ptr->l1 / L1_MAX * 0x7FFF );	// control surface commands (normalized 0-1)
	tele_data[14]= (uint16_t)( controlData_ptr->l2 / L2_MAX * 0x7FFF );
	tele_data[15]= (uint16_t)( controlData_ptr->l3 / L3_MAX * 0x7FFF );
	tele_data[16]= (uint16_t)( controlData_ptr->l4 / L4_MAX * 0x7FFF );
	tele_data[17]= (uint16_t)( controlData_ptr->r1 / R1_MAX * 0x7FFF );
	tele_data[18]= (uint16_t)( controlData_ptr->r2 / R2_MAX * 0x7FFF );
	tele_data[19]= (uint16_t)( controlData_ptr->r3 / R3_MAX * 0x7FFF );
	tele_data[20]= (uint16_t)( controlData_ptr->r4 / R4_MAX * 0x7FFF );
	tele_data[21]= (uint16_t)( controlData_ptr->dthr * 0x7FFF );
	...
	tele_data[33] = cpuLoad;
	
	tele_data[34] = (uint16_t)sensorData_ptr->gpsData_ptr->satVisible;
